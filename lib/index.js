...
function setupApp () {
  ...
  app.post('/elmis-orders', (req, res) => {
    ...
    request.post(options, (err, response, body) => {
      ...
      let orchestrations = []
      let urlResponse = `http://localhost:7000/elmis-orders?sourceApplication=${req.body.sourceApplication}&rnrId=${req.body.rnrId}`
      orchestrations.push(utils.buildOrchestration('Capturing Orchestration Data Route', new Date().getTime(), req.method, urlResponse, req.headers, JSON.stringify(req.body), orchestrationResponse, body))
